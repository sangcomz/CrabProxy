# Crab Proxy One-Shot 전환 계획서 v4 (Quality First)
...
- 멀티 호스트 분산 제어
- FFI 병행 운영
- 기존 `crab-mitm` CLI 바이너리(`run`/`ca` 서브커맨드) 유지: `crabctl`과 `crabd`가 대체하며, 전환 완료 시 `crab-mitm/src/main.rs`는 삭제
...
### 5.5 helper.* API 접근 제어

- `helper.*` 메서드는 권한이 높은 시스템 작업이므로 클라이언트 타입별 접근 제어 적용
- 허용 정책:

| 메서드 | App | CLI | MCP |
|--------|-----|-----|-----|
| `helper.enable_pf` | O | O | X |
| `helper.disable_pf` | O | O | X |
| `helper.install_cert` | O | O | X |
| `helper.remove_cert` | O | O | X |
| `helper.check_cert` | O | O | O |

- MCP는 `helper.check_cert`(읽기 전용)만 허용, 나머지 시스템 변경 작업은 거부
- `system.handshake` 시 전달하는 `client_type` 값(`app`, `cli`, `mcp`)으로 구분
...
- 기본 정책: proxy 실행 중이면 daemon 유지 (클라이언트 유무 무관)
- proxy 정지 상태 + 클라이언트 0개인 채로 idle timeout(10분) 경과 시 자동 종료
  - launchd `KeepAlive = false` 설정으로 자동 종료 후 재기동 안 함
  - 다음 `launchctl kickstart` 시 다시 시작
- 명시적 종료: `daemon.shutdown` (proxy 실행 중에도 강제 종료)
- idle timeout은 `crabd --idle-timeout <seconds>` 로 조정 가능 (0 = 비활성화)

### 장애 복구

- launchd가 crash restart 담당 (`KeepAlive.Crashed = true`)
- 클라이언트 재연결 전략:
  - 최대 재시도: 10회
...
| `system.handshake` | 세션 수립 | `{ "protocol_version": 1, "client_type": "app"\|"cli"\|"mcp", "token": "..." }` | `{ "session_id": "...", "protocol_version": 1 }` |
...
**`config.patch`**: 부분 설정 변경 (JSON Merge Patch, RFC 7396)

규칙:
- 전달된 키만 변경, 생략된 키는 기존 값 유지
- 명시적으로 `null`을 전달하면 해당 키를 기본값으로 초기화
...
### 정책: Last-Write-Wins + 변경 알림

- 동시에 여러 클라이언트가 설정/룰을 변경할 수 있음
- daemon은 **마지막 도착 요청을 적용** (Last-Write-Wins)
...
- 향후 확장 예약: 낙관적 잠금 (`config.patch`에 `expected_revision` 옵션 파라미터 추가 가능)
  - 현재는 구현하지 않으나, 응답에 `revision`을 항상 포함하여 향후 호환 가능하게 유지

### 경합 시나리오

| 시나리오 | 동작 | 비고 |
|---------|------|------|
| 앱과 CLI가 동시에 룰 변경 | 나중에 도착한 요청이 최종 적용 | 두 클라이언트 모두 `state.changed` 수신 |
...
- 완료 마크: `migration.v4.completed = true` (daemon 저장소에 기록)
...
2. daemon에 `migration.v4.completed` 확인
3. 미완료인 경우: 앱이 `UserDefaults`에서 설정 읽기 → `config.patch` + `rules.apply`로 daemon에 전달
4. daemon이 저장 후 `migration.v4.completed = true` 기록
...
- ADR-001/002/003 승인
- API 스펙 v4 최종 확정 (`config.patch` RFC 7396 규칙 포함)
- 에러 코드 체계 확정
...
- `config.patch`가 RFC 7396 규칙대로 동작함
...
| 다중 클라이언트 충돌 | Last-Write-Wins + `state.changed` 브로드캐스트, 향후 `expected_revision` 확장 가능 |
| SQLite 손상 | WAL 모드 + 정기 `PRAGMA integrity_check` (doctor 명령), 복구 불가 시 초기화 |
| MCP 보안 오남용 | 클라이언트 타입별 `helper.*` 접근 제어, `PERMISSION_DENIED` 에러 |
...
- v3 대비 보강된 부분: XPC 브리지 구현 방식 확정, 에러 코드 체계, 다중 클라이언트 동시성 정책, config.patch 명세, 로그 운영 정책, 클라이언트 타입별 접근 제어.
