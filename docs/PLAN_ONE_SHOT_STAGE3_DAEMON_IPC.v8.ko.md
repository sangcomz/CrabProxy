# Crab Proxy One-Shot 전환 계획서 v8 (Quality First)
...
- D2. IPC 인증: `UDS 권한 + HMAC scope 토큰 + peer pid/code-signing principal` 검증
...
- Compatibility Discipline: 버전 협상 + 스키마 진화 정책을 코드로 강제
...
### ADR-002: UDS + HMAC scope 토큰 + peer principal 인증

결정:
- IPC 인증을 파일 권한 기반 + **HMAC scope 토큰** + **peer principal(코드서명) 검증**으로 구성
...
- 소켓 권한: `0600`
- peer credential 검사:
  - `LOCAL_PEERCRED`로 peer uid 획득
  - `LOCAL_PEERPID`로 peer pid 획득
  - pid 기반 코드서명 정보 조회로 principal(`app`/`cli`/`mcp`) 판별
...
| 필요 scope | 메서드 |
|------------|--------|
| `read` | `system.ping`, `system.version`, `proxy.status`, `config.get`, `rules.get`, `logs.subscribe`, `logs.unsubscribe`, `logs.tail`, `ca.status`, `daemon.doctor` |
| `rules.write` | `rules.patch` |
| `control` | `config.patch`, `rules.apply`, `proxy.start`, `proxy.stop`, `proxy.replay`, `ca.load`, `ca.generate`, `logs.clear` |
...
- `aud`: 토큰 대상 principal (`app`/`cli`/`mcp`)
...
| 파일 | 경로 | 포함 scopes |
|------|------|-------------|
| `app.token` | `~/Library/Application Support/CrabProxy/run/app.token` | `read`, `rules.write`, `control`, `admin` (`aud=app`) |
| `cli.token` | `~/Library/Application Support/CrabProxy/run/cli.token` | `read`, `rules.write`, `control`, `admin` (`aud=cli`) |
| `mcp.token` | `~/Library/Application Support/CrabProxy/run/mcp.token` | `read`, `rules.write` (`aud=mcp`) |
...
3. 3개 토큰 파일 각각에 대해:
   - payload JSON 구성 (`aud`, scopes, iat, jti)
   - HMAC-SHA256 서명 생성
...
- `system.rotate_token` 호출 시:
  ...
  - 호출자에게 먼저 `{ "rotated": true, "reconnect_required": true }` 응답
  - 응답 flush 후 호출자 포함 모든 세션 종료 (`SESSION_EXPIRED`)
  - 클라이언트는 새 토큰 파일을 읽어 재연결
  - `admin` scope 필요

### 5.5 코드서명 기반 Principal 검증

#### 개요

`system.handshake` 시 peer pid에서 코드서명 identity를 조회하여 principal(`app`/`cli`/`mcp`)을 판별하고, 토큰의 `aud` 필드와 일치하는지 검증한다.

#### 구현 접근법 (Gate 0 PoC에서 비교 평가)

| 접근법 | 설명 | 장점 | 단점 |
|--------|------|------|------|
| A안: `security-framework` 크레이트 | Rust에서 Security.framework 바인딩을 통해 `SecCodeCopyGuestWithAttributes` + `SecCodeCopySigningInformation` 호출 | 순수 Rust, 외부 프로세스 불필요 | API가 unsafe, 코드서명 세부사항 직접 처리 필요 |
| B안: `objc`/`objc2` 크레이트 | Objective-C 런타임으로 Security.framework 직접 호출 | A안과 유사하나 더 low-level 제어 가능 | unsafe 코드 증가, 유지보수 부담 |
| C안: `crabd-xpc-bridge` 위임 | 기존 XPC 브리지 바이너리에 코드서명 검증 명령 추가 (`verify_codesign { pid: N }`) | Swift에서 Security.framework 자연스럽게 호출, Rust 복잡도 최소화 | 매 handshake마다 subprocess I/O 발생 (지연 ~10ms) |

- **Gate 0에서 A안 우선 프로토타입**, 복잡도가 수용 불가능하면 C안으로 전환
- C안 채택 시 `crabd-xpc-bridge`에 `verify_codesign` 명령 추가 (XPC Helper 호출 없이 bridge 자체에서 처리)

#### Gate 정책 (No Fallback)

코드서명 기반 principal 검증은 release 품질 게이트의 필수 항목이다.

- Gate 0 PoC 불합격 시: 출시를 진행하지 않고 구현 방식을 A안→C안으로 전환해 재검증
- Gate 1 종료 기준: `system.handshake` 성공 케이스는 모두 `principal_verified=true`여야 함
- release 빌드에서 코드서명 principal 검증을 우회하는 경로는 허용하지 않음

#### Gate 0 코드서명 PoC 판단 기준

| 기준 | 합격 | 불합격 시 조치 |
|------|------|----------------|
| pid → 코드서명 identity 조회 | 10ms 이내 응답 | A안 실패 시 C안으로 전환 후 재검증 |
| 서명 없는 바이너리 처리 | 명확한 에러 반환 | `PRINCIPAL_MISMATCH`로 fail-closed 고정 |
| 개발 빌드(ad-hoc 서명) 처리 | identifier 기반 매칭 가능 | 개발용 서명 파이프라인 정비 후 재검증 |
| Rust 구현 복잡도 | 200 LOC 이내 (A안 기준) | 복잡도 초과 시 C안으로 전환 |

### 5.6 Principal 식별자 매핑

#### 매핑 테이블

peer pid의 코드서명 identity에서 추출한 식별자를 아래 테이블에 따라 principal로 매핑한다.

| 코드서명 식별자 (Bundle ID / Signing Identity) | principal | 비고 |
|------------------------------------------------|-----------|------|
| `com.sangcomz.CrabProxyMacApp` | `app` | macOS 앱 번들 (`PRODUCT_BUNDLE_IDENTIFIER`) |
| `com.sangcomz.crabctl` | `cli` | CLI 바이너리 |
| `com.sangcomz.crab-mcp` | `mcp` | MCP 서버 바이너리 |
| `com.sangcomz.CrabProxy.crabd-xpc-bridge` | (내부 전용) | XPC 브리지, IPC 클라이언트로 접속하지 않음 |

- 매핑은 `crabd` 내부 설정으로 하드코딩 (외부 설정 파일 불필요)
- 매핑 테이블에 없는 식별자 → `PRINCIPAL_MISMATCH` 반환
- 동일 team identifier(`XXXXXXXXXX`)로 서명된 바이너리만 매핑 대상
- 매핑 테이블 값은 릴리즈 CI에서 `codesign -dv --verbose=4` 출력과 대조해 drift를 차단

#### 개발 빌드 정책

| 빌드 유형 | 코드서명 상태 | principal 판별 방식 |
|-----------|---------------|---------------------|
| Release (배포) | Apple Developer ID 서명 + Notarization | Bundle ID 기반 매핑 (위 테이블) |
| Debug (Xcode) | Ad-hoc 서명 (자동) | `codesign --identifier`에 설정된 identifier 기반 매핑 |
| Cargo 빌드 (Rust CLI) | Ad-hoc 서명(개발 스크립트) | identifier 기반 매핑 |

**서명 없는 바이너리 처리 정책**:

- debug/release 공통으로 서명 없는 바이너리는 `PRINCIPAL_MISMATCH` 반환 (fail-closed)
- 개발 환경에서는 ad-hoc 서명 스크립트로 `crabctl`/`crab-mcp` identifier를 주입해 실행
...
3. `LOCAL_PEERPID`로 peer pid 획득
4. peer pid의 코드서명 identity를 검증해 principal(`app`/`cli`/`mcp`) 판별 (**5.5 참조**)
5. 토큰 HMAC 서명 검증 통과
6. 토큰 `aud`와 peer principal 일치 확인 (코드서명 기반 principal과 일치해야 함)
7. 토큰 `jti`가 폐기 목록에 없음 확인
8. `protocol_version` 호환 통과

하나라도 실패하면 즉시 연결 거부. 세션 수립 후 daemon은 `{principal, scopes, principal_verified}`를 세션에 기록하고, 이후 모든 API 호출에서 scope 검사를 수행한다.

`client_type` 필드는 선택적 진단 정보이며 권한판단에 사용하지 않는다.
...
- idle auto-exit 정책은 v8 범위에서 제외 (생명주기 단순화/예측가능성 우선)
...
- **기존 사용자 데이터 안내**: v8 전환 시 기존 `UserDefaults` 기반 설정/룰은 새 daemon 아키텍처로 이전되지 않음 (섹션 12 참조). 업데이트 전 릴리즈 노트에서 이 사항을 명확히 고지
...
- 이유: 원자성, 크래시 복구, 스키마 진화 일관성

### 7.2 스키마 버전

- `schema_version` 테이블로 명시 관리
- 앱/daemon 버전과 독립적으로 DB 스키마 업데이트 실행
...
- 출시 전 전환 기준으로 기존 `UserDefaults` 룰은 import 하지 않음
- 초기 상태는 빈 룰 배열(`allow`, `map_local`, `map_remote`, `status_rewrite`)로 시작
- 새 룰 생성 시 daemon이 UUID를 자동 부여
...
| `system.handshake` | (없음) | 세션 수립 | `{ "protocol_version": 1, "token": "...", "client_type?": "app"\|"cli"\|"mcp" }` | `{ "session_id": "...", "protocol_version": 1, "principal": "mcp", "scopes": ["read", "rules.write"], "principal_verified": true }` |
| `system.rotate_token` | `admin` | 토큰 전체 회전 | - | `{ "rotated": true, "reconnect_required": true }` |

- `principal_verified`: 코드서명 기반 principal 검증 수행 여부. 본 스펙에서는 handshake 성공 시 항상 `true`.
...
- 명시적으로 `null`을 전달하면 해당 키를 기본값으로 초기화 (v7 확장 규칙, RFC 7396과 다름에 주의)
...
| `logs.clear` | `control` | 로그 전체 삭제 | - | `{ "cleared": true }` |
...
| `daemon.doctor` | `read` | 진단 정보 수집 | - | `{ "socket": "ok", "state_db": "ok", "helper_xpc": "ok", "ca": "loaded", "launch_agent": "registered", "principal_verification": "enabled", ... }` |

- `principal_verification`: 코드서명 기반 principal 검증 상태 (`enabled`여야 정상. 그 외 상태는 릴리즈 블로커)
...
아래 표의 `응답` 컬럼은 JSON-RPC `result` 오브젝트 내부 payload를 의미한다.
...
│       ├── principal.rs              # 코드서명 기반 principal 검증 (5.5/5.6 참조)
│       └── error.rs                  # 에러 코드 및 타입
...
- 출시 전 전환 기준으로 기존 `UserDefaults` 설정/룰은 import 하지 않음
- daemon 첫 실행 시 state DB가 없으면 기본 상태를 생성하고 normal mode로 시작
- 초기 룰 상태는 빈 배열(`allow`, `map_local`, `map_remote`, `status_rewrite`)
- 룰은 생성 시 UUID 자동 부여

부트스트랩 흐름:
1. 앱/CLI/MCP가 daemon에 첫 연결
2. daemon이 state DB 존재 여부 확인
3. DB가 없으면 기본 설정 + 빈 룰 상태 생성
4. 생성 성공 후 normal mode 유지
5. 클라이언트는 `config.get`/`rules.get`으로 최신 상태 동기화

실패 처리:
- 초기 상태 생성 실패 시 `STATE_BOOTSTRAP_FAILED` 반환
- 오류 원인은 `daemon.doctor`에서 확인 가능해야 함

### 기존 사용자 데이터 처리

v8 전환은 clean bootstrap 전략을 채택하며, 기존 `UserDefaults` 기반 데이터는 새 아키텍처로 이전하지 않는다.

**영향받는 데이터**:
- `UserDefaults`에 저장된 프록시 설정 (listen 주소/포트, throttle, inspect 등)
- `UserDefaults`에 저장된 룰 (allow, map_local, map_remote, status_rewrite)
- `UserDefaults`에 저장된 클라이언트 allowlist

**사용자 고지 계획**:

| 시점 | 조치 |
|------|------|
| 업데이트 릴리즈 노트 | "기존 설정/룰이 초기화됩니다. 업데이트 전 필요한 설정을 별도 메모해 두세요." 명시 |
| 앱 내 업데이트 안내 (가능 시) | 앱 업데이트 전 in-app 배너로 데이터 초기화 사전 고지 |
| 최초 실행 시 | daemon 첫 기동 시 빈 상태로 시작되며, "새로운 아키텍처로 전환되었습니다" 안내 표시 |
| 프로젝트 README / 변경 이력 | Breaking Change로 명시, 마이그레이션 없음을 기술 |
...
- ADR-001/002/003 승인
- API 스펙 v8 최종 확정 (scope 매핑, principal 검증, 토큰 형식, 룰 ID 포함)
- 에러 코드 체계 확정
- Helper 보안 정책(허용 식별자/요구사항) 확정
- **XPC 브리지 프로토타입 구현** (A안 Swift 브리지 vs B안 objc 크레이트 비교)
- JSON-RPC 라이브러리 프로토타입 (자체 구현 vs `jsonrpsee` 비교 → 확정)
- HMAC 토큰 생성/검증 프로토타입
- **코드서명 기반 principal 검증 PoC** (5.5절 참조):
  - A안(`security-framework` 크레이트) 우선 프로토타입
  - 판단 기준(5.5절 테이블) 기반 합격/불합격 결정
  - 불합격 시 → C안(`crabd-xpc-bridge` 위임)으로 전환 후 재검증
  - 합격 시 → principal 매핑 테이블(5.6절) 동작 검증 포함

산출물:
- API 스펙 v8 확정본
- 보안 모델 문서 (scope 매핑, principal 검증, 토큰 형식 포함)
- 상태/로그 저장 스키마 문서
- 룰 ID 체계 확정 문서
- XPC 브리지 PoC + 방식 확정 결과
- **코드서명 검증 PoC 결과 보고서** (합격/불합격 판정 + 채택 접근법 결정)
...
- handshake + peer pid/code-signing principal 검증 구현 (코드서명 검증 필수)
- `crab-ipc/src/principal.rs` 구현 (매핑 테이블 + 개발 빌드 정책)
- `crabd-xpc-bridge` 구현

산출물:
- IPC 통합 테스트
- 인증/scope/principal 실패 케이스 테스트 (wrong scope → PERMISSION_DENIED, aud mismatch → PRINCIPAL_MISMATCH)
- XPC 브리지 통합 테스트
- principal 검증 테스트 (등록 식별자/미등록 식별자/미서명 바이너리)
...
- 초기 상태 부트스트랩(first-run) 검증
- scope 기반 접근 제어 검증 (MCP에서 `rules.patch` 성공, `rules.apply`/`logs.clear` 거부)
- **기존 사용자 데이터 고지 문구 준비** (릴리즈 노트, 앱 내 안내, README 변경 이력)
...
- **사용자 고지 문구 최종 확인** (릴리즈 노트, 앱 내 안내)

산출물:
- release candidate 품질 리포트
- blocker 0개 확인
- 삭제 대상 파일 정리 확인
- 릴리즈 노트 최종본 (기존 데이터 초기화 안내 포함)
...
- 토큰 `aud`와 peer principal 불일치 → `PRINCIPAL_MISMATCH`
- `mcp.token`으로 `rules.apply` 호출 → `PERMISSION_DENIED`
- `mcp.token`으로 `logs.clear` 호출 → `PERMISSION_DENIED`
...
- 알려진 식별자 → 올바른 principal 매핑, 미등록/미서명 식별자 → `PRINCIPAL_MISMATCH`
...
| 단계 | 기간 | 비고 |
|------|------|------|
| Gate 0: 설계 고정 | 3일 | XPC 브리지 PoC + HMAC 프로토타입 + **코드서명 검증 PoC** 포함 |
| Gate 1: 플랫폼 기반 | 7일 | workspace + IPC + 토큰/scope + XPC 브리지 + principal 검증 |
| Gate 2: 엔진/권한 통합 | 8일 | 룰 ID + 동시성 테스트 포함 |
| Gate 3: 클라이언트 전환 | 6일 | 초기 상태 부트스트랩 + scope 접근 제어 + **사용자 고지 준비** |
| Gate 4: 하드닝 | 7일 | FFI 삭제 + 토큰 회전 + 최종 검증 + **릴리즈 노트 확정** |
| **총합** | **약 4.5~5.5주** | |
...
- `system.handshake` 실패가 fail-closed로 동작함 (uid, pid/code-signing principal, HMAC, protocol_version 검증)
- principal 검증이 release에서 항상 활성 상태로 동작함 (`principal_verified=true`, 우회 경로 없음)
- MCP에서 `rules.patch` 성공, `rules.apply`/`helper.*`/`config.patch`/`logs.clear` 거부됨
- `config.patch`가 RFC 7396 기반 확장 규칙(v7, `null`=기본값 복원)대로 동작함
- 룰 ID(UUID) 체계가 동작하고, `rules.patch` upsert/remove가 정상 작동함
- 다중 클라이언트 동시 접근 시 `state.changed` 브로드캐스트 및 일관성 유지
- 토큰 회전 시 호출자 성공 응답 후 전체 세션 만료 및 재연결 동작
- 로그 저장소 크기 제한/로테이션 동작
- 초기 상태 부트스트랩/복구/부하/보안 테스트가 게이트 기준을 통과함
- **기존 사용자 데이터 초기화에 대한 고지가 릴리즈 노트, 앱 내 안내, 변경 이력에 반영됨**
...
| MCP 보안 오남용 | 코드서명 기반 peer principal + HMAC scope 토큰 + `aud` 일치 검증으로 권한 승격 차단 |
| 토큰 유출 | 파일 권한 `0600`, 코드서명 principal 검증, HMAC 검증, `aud`-principal 일치 검증, 유출 감지 시 `system.rotate_token`으로 즉시 무효화 |
| LaunchAgent 좀비화 | `daemon.doctor`에 LaunchAgent 상태 포함, 앱 제거 가이드에 plist 삭제 절차 명시 |
| 코드서명 검증 기술 장벽 | Gate 0 PoC에서 조기 판단, A안 불합격 시 C안으로 전환해 재검증. 코드서명 principal 검증 완료 전 release 금지 |
| **기존 사용자 데이터 유실** | **clean bootstrap 전략 채택으로 기존 UserDefaults 설정/룰 초기화됨. 릴리즈 노트, 앱 내 안내, README 변경 이력에 사전 고지** |
...
- v7 대비 보강된 부분: **코드서명 검증 PoC를 Gate 0에 명시하고 release 필수 게이트로 고정(No Fallback)**, **principal 식별자 매핑 테이블 및 개발 빌드 정책 정밀화(앱 식별자 정정 + CI drift 검증)**, **기존 사용자 데이터 유실 리스크 고지 계획 수립 (릴리즈 노트, 앱 내 안내)**.
