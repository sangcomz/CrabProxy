# Crab Proxy One-Shot 전환 계획서 v7 (Quality First)
...
4. peer pid의 코드서명 identity를 검증해 principal(`app`/`cli`/`mcp`) 판별
5. 토큰 HMAC 서명 검증 통과
6. 토큰 `aud`와 peer principal 일치 확인
7. 토큰 `jti`가 폐기 목록에 없음 확인
8. `protocol_version` 호환 통과

하나라도 실패하면 즉시 연결 거부. 세션 수립 후 daemon은 `{principal, scopes}`를 세션에 기록하고, 이후 모든 API 호출에서 scope 검사를 수행한다.
...
- idle auto-exit 정책은 v7 범위에서 제외 (생명주기 단순화/예측가능성 우선)
...
| `system.handshake` | (없음) | 세션 수립 | `{ "protocol_version": 1, "token": "...", "client_type?": "app"\|"cli"\|"mcp" }` | `{ "session_id": "...", "protocol_version": 1, "principal": "mcp", "scopes": ["read", "rules.write"] }` |
...
- ADR-001/002/003 승인
- API 스펙 v7 최종 확정 (scope 매핑, principal 검증, 토큰 형식, 룰 ID 포함)
- 에러 코드 체계 확정
...
- handshake + peer pid/code-signing principal 검증 구현
- `crabd-xpc-bridge` 구현
...
| Gate 3: 클라이언트 전환 | 6일 | 초기 상태 부트스트랩 + scope 접근 제어 |
...
| MCP 보안 오남용 | HMAC scope 토큰 + peer principal(`aud`) 바인딩으로 권한 승격 차단 |
| 토큰 유출 | 파일 권한 `0600`, HMAC 검증, `aud`-principal 일치 검증, 유출 감지 시 `system.rotate_token`으로 즉시 무효화 |
...
- v6 대비 보강된 부분: peer pid/code-signing principal 검증, token `aud` 바인딩, `logs.clear` 권한 상향(`read`→`control`), `rotate_token` 호출자 응답/만료 순서 고정, 응답 스키마 단일화, legacy import 없는 초기 상태 부트스트랩으로 단순화.
