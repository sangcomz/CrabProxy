...
protocol MacSystemProxyServicing: Sendable {
...
struct LiveMacSystemProxyService: MacSystemProxyServicing {
  func readStatus() throws -> MacSystemProxyStatus {
    try MacSystemProxyService.readStatus()
  }

  func enable(host: String, port: Int) throws -> MacSystemProxyStatus {
    try MacSystemProxyService.enable(host: host, port: port)
  }
...
enum MacSystemProxyService {
  ...
  private static func removeServicePrefixMarker(_ line: String) -> String {
    guard let close = line.firstIndex(of: ")") else {
      return line.replacingOccurrences(of: "*", with: "").trimmingCharacters(
